...
The added annotations will be checked *each* time that `admins` is updated at a
specific index `i`. Note that this annotations while faster (there is no loop as
there would be with an `if_updated forall...`), may miss some cases. For example
if the code adds an admin with `admins.push(newAdmin)`, that site won't be
instrumented with `#if_assigned`. (while it will be instrumented with
`#if_updated`). Therefore always use `#if_assigned` with care, and wherever possible, use `#if_updated` over `#if_assigned`
